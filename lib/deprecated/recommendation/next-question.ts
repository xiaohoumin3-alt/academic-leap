...
async function getQuestionsByKnowledgePoint(
  ...
  // Fetch all questions and filter in memory (Json field doesn't support contains)
  ...
  // Filter by knowledgePoint (Json array)
  const filtered = allQuestions.filter(q => {
    const kps = q.knowledgePoints;
    if (Array.isArray(kps)) {
      return kps.some(kp => {
        if (typeof kp === 'string') return kp === knowledgePoint || kp.includes(knowledgePoint);
        if (typeof kp === 'object' && kp !== null) {
          const name = (kp as { name?: string }).name || (kp as { id?: string }).id || '';
          return name === knowledgePoint || name.includes(knowledgePoint);
        }
        return false;
      });
    }
  ...
  return filtered.slice(0, 100).map(q => ({
    id: q.id,
    knowledgePoint,
    difficultyLevel: q.difficulty,
    content: {
      // content is now Json type - extract question text
      question: typeof q.content === 'object' && q.content !== null
        ? (q.content as { question?: string }).question || JSON.stringify(q.content)
