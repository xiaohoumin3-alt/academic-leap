...
export async function POST(req: NextRequest) {
  try {
    ...
    // 如果没有有效答题记录，返回明确的业务错误而非继续处理
    if (answerRecords.length === 0) {
      console.log('[Assessment Finish] No valid answerRecords');
      return NextResponse.json(
        { error: '未获取到有效的答题记录，无法计算成绩' },
        { status: 400 }
      );
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      // 幂等性检查：如果该 attempt 已经有 score，说明已经计算过
      ...
      if (existingAttempt?.completedAt) {
        ...
        // attempt 已完成但没有 assessment 记录，这是异常状态
        return {
          assessmentId: null,
          learningPathId: null,
          scoreResult,
          isExisting: true,
          error: 'attempt_completed_no_assessment'
        };
      }
      ...
      // 返回 assessment ID 和 learningPathId
      return {
        assessmentId: assessment.id,
        learningPathId,
      };
    ...
    const createdAssessmentId = result.assessmentId;
    const autoGeneratedLearningPathId = result.learningPathId;

    // 幂等性检查：attempt 已完成但无 assessment 记录的情况
    if (!createdAssessmentId && result.isExisting) {
      return NextResponse.json(
        { error: '测评已完成但记录异常，请联系管理员', success: false },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        learningPathId: autoGeneratedLearningPathId,  // 自动生成的学习路径 ID（60-89分时）
        // 诊断决策
        accuracy,
        adaptiveAction,
        // 等效分相关
